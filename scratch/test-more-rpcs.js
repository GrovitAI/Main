...
async function testRpc(name, body) {
  ...
  try {
    ...
    console.log(`RPC ${name} response:`, data.substring(0, 300));
  } catch (err) {
...
async function run() {
  await testRpc('query', { query: 'SELECT 1;' });
  await testRpc('sql', { query: 'SELECT 1;' });
  await testRpc('sql', { sql: 'SELECT 1;' });
  await testRpc('run_query', { query: 'SELECT 1;' });
  await testRpc('run_query', { sql: 'SELECT 1;' });
  await testRpc('execute_query', { query: 'SELECT 1;' });
}
